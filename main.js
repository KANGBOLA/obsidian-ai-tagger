...
var AITaggerPlugin = class extends import_obsidian.Plugin {
  ...
  async onload() {
    ...
    this.initClient();
    this.addRibbonIcon("tag", "AI Tagger", async (evt) => {
  ...
  async tagCurrentNote() {
    if (!this.client) {
      new import_obsidian.Notice("\u26A0\uFE0F Please set your Claude API key in settings");
      return;
    }
    ...
    if (!file)
      return;
    new import_obsidian.Notice("\u{1F916} Analyzing note...");
    try {
      const content = await this.app.vault.read(file);
      const tags = await this.suggestTags(content);
      if (tags.length === 0) {
        new import_obsidian.Notice("No tags suggested");
        return;
      }
  ...
  async tagAllNotes() {
    ...
    new import_obsidian.Notice(`\u{1F916} Tagging ${files.length} notes...`);
    let processed = 0;
    for (const file of files) {
      try {
        ...
        processed++;
        if (processed % 10 === 0) {
          new import_obsidian.Notice(`Progress: ${processed}/${files.length}`);
        }
      } catch (error) {
        console.error(`Error tagging ${file.path}:`, error);
      }
    }
    new import_obsidian.Notice(`\u2705 Done! Tagged ${processed} notes`);
  }
  async suggestTags(content) {
    if (!this.client)
      return [];
    const truncated = content.slice(0, 3e3);
    const langHint = this.settings.language === "ko" ? "Respond with Korean tags." : this.settings.language === "en" ? "Respond with English tags." : "Detect the language of the content and use the same language for tags.";
  ...
  async applyTags(file, tags) {
    ...
    if (match) {
      ...
      if (existingTags) {
        ...
        const merged = [.../* @__PURE__ */ new Set([...current, ...tags])];
...
var TagSuggestionModal = class extends import_obsidian.Modal {
  constructor(app, tags, file, plugin) {
    super(app);
    this.tags = tags;
    this.file = file;
    this.plugin = plugin;
  ...
  onOpen() {
    ...
    contentEl.createEl("h2", { text: "\u{1F3F7}\uFE0F Suggested Tags" });
    ...
    applyBtn.onclick = async () => {
      ...
      new import_obsidian.Notice(`\u2705 Applied ${this.selected.size} tags`);
      this.close();
    };
  }
  onClose() {
...
var AITaggerSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Claude API Key").setDesc("Get your API key at console.anthropic.com").addText((text) => text.setPlaceholder("sk-ant-...").setValue(this.plugin.settings.apiKey).onChange(async (value) => {
      this.plugin.settings.apiKey = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Model").setDesc("Claude model to use (Haiku is fastest and cheapest)").addDropdown((drop) => drop.addOption("claude-haiku-4-5-20251001", "Claude Haiku (Fast & Cheap)").addOption("claude-sonnet-4-6", "Claude Sonnet (Balanced)").setValue(this.plugin.settings.model).onChange(async (value) => {
      this.plugin.settings.model = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Max Tags").setDesc("Maximum number of tags to suggest per note").addSlider((slider) => slider.setLimits(1, 10, 1).setValue(this.plugin.settings.maxTags).setDynamicTooltip().onChange(async (value) => {
      this.plugin.settings.maxTags = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Language").setDesc("Language for generated tags").addDropdown((drop) => drop.addOption("auto", "Auto-detect").addOption("en", "English").addOption("ko", "Korean (\uD55C\uAD6D\uC5B4)").setValue(this.plugin.settings.language).onChange(async (value) => {
      this.plugin.settings.language = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Auto-apply tags").setDesc("Automatically apply tags without confirmation (for bulk tagging)").addToggle((toggle) => toggle.setValue(this.plugin.settings.autoApply).onChange(async (value) => {
      this.plugin.settings.autoApply = value;
      await this.plugin.saveSettings();
    }));
    containerEl.createEl("div", {
      text: "\u{1F496} If this plugin saves you time, please consider sponsoring!",
